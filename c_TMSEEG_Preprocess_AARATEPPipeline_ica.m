function EEG = c_TMSEEG_Preprocess_AARATEPPipeline(varargin)
	...
	p.addParameter('artifactTimespan', [-0.002, 0.020], @c_isSpan);
	p.addParameter('baselineTimespan', [-0.5 -0.02], @c_isSpan);
	...
	p.addParameter('badChannelThreshold', 8, @isscalar);
    p.addParameter('IMPChannels', {'f5','f3','fc3','fc5','fp1','fp2','fpz'}, @(x) iscellstr(x) || isnumeric(x));
	p.addParameter('notch_freq', [58 62], @c_isSpan);
	...
	p.addParameter('ICAType', 'fastica', @ischar); % picard fastica
	...
	EEG.report=table;
    EEG.report.file={EEG.filename};
    EEG.report.epochs=EEG.trials;
	%     if s.doDebug
	...
	%% downsample
	EEG.report.orgsamprate=EEG.srate;
	...
	EEG.report.newsamprate=EEG.srate;
	%% baseline correct
    ...
    if ~isempty(misc.badChannelIndices)
	    [te EEG.report.remchan]=elecName(orgEEG,{misc.badChannelIndices}');
    elseif isempty(misc.badChannelIndices)
        EEG.report.remchan={''};
    end
	...
	if s.initialEyeComponentThreshold < 1
		...
		EEG = c_EEG_ICA(EEG, 'method', 'picard');  %fastica picard
		c_sayDone();
		%2nd ICA
		...
		[EEG, misc] = c_TMSEEG_runICLabel(EEG,...
            ...
            'lineNoiseComponentThreshold', 0.85,...
			'doPlot', true,...
			'doRejection', false); 
		c_sayDone();
        ...
        %EEG.report.eyeICA_numComp=md.eyeICA_numComp;
        %EEG.report.eyeICA_numRejComp=md.eyeICA_numRejComp;
	...
	if false
		EEG = pop_eegfiltnew(EEG, s.notch_freq(1), s.notch_freq(2), 2*EEG.srate, 1);
	else
		EEG = c_EEG_filter_butterworth(EEG, s.notch_freq, 'type', 'bandstop');
	end
	c_sayDone();
	...
	EEG.report.ICA_miscComp={string([find(misc.rejectComponents)'])};
    EEG.report.ICA_miscNumComp={sum(misc.rejectComponents)};
    ...
    EEG.report.ICA_numComp=md.ICA_numComp;
    EEG.report.ICA_numRejComp=md.ICA_numRejComp;
